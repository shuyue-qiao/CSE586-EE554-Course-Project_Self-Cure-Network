...
# Pre-process datasets for later training or testing
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform = None, basic_aug = False):
        ...
        # Note: Label values are 1 less than the original datasets labels
        # Now 0:Surprise, 1:Fear, 2:Disgust, 3:Happiness, 4:Sadness, 5:Anger, 6:Neutral
        self.label = dataset.iloc[:, LABEL_COLUMN].values - 1
        
        # Collect locations of the images for later training or testing
        self.file_paths = []
        for f in file_names:
            f = f.split(".")[0]
            f = f +"_aligned.jpg"
    ...
    def __getitem__(self, idx):
        ...
        # Use sef-defined functions for data augmentation
        if self.phase == 'train':
...
class Res18Feature(nn.Module):
    def __init__(self, pretrained = True, num_classes = 7, drop_rate = 0):
        ...
        fc_in_dim = list(resnet.children())[-1].in_features # original fc layer in dimention 512
        self.fc = nn.Linear(fc_in_dim, num_classes) # new fc layer 512x7
        # A linear fully-connected (FC) layer and a sigmoid activation function are used
...
def run_training():
    ...
    # Use ResNet-18 as the CNN backbone for face features extraction
